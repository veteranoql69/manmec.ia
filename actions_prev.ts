...
/**
 * Obtiene todas las OTs de la organizaci├│n
 */
...
/**
 * Obtiene una OT espec├¡fica por ID con el contexto completo (Equipo, Insumos, Veh├¡culo)
 */
export async function getWorkOrderDetail(id: string) {
    const profile = await requireRole("SUPERVISOR");
    const supabase = await createClient();

    // 1. Obtener la OT b├ísica con Estaci├│n y Veh├¡culo
    ...
    // 2. Obtener el Equipo HUMANO (M├║ltiples mec├ínicos)
    ...
    // 4. Obtener el inventario "Live Stock" y herramientas del Furg├│n asignado si lo hay
    let mobileWarehouse = { id: null, stock: [], tools: [] };

    if (ot.vehicle_id) {
        // Encontrar la bodega m├│vil del veh├¡culo
        ...
        // Herramientas Asignadas al veh├¡culo (v├¡a su Bodega M├│vil)
        if (warehouse) {
...
/**
 * Asigna un mec├ínico a una OT (Como L├¡der o Apoyo)
 */
export async function assignMechanicToWorkOrder(otId: string, mechanicId: string, role: "lead" | "support" = "support") {
    ...
    // Si es l├¡der, actualizar el campo assigned_to de la OT principal
    if (role === "lead") {
...
/**
 * Desasigna un mec├ínico de una OT
 */
export async function unassignMechanicFromWorkOrder(otId: string, mechanicId: string) {
    ...
    // 2. Si era el l├¡der (assigned_to), limpiar el campo en la OT principal
...
/**
 * Desasigna un veh├¡culo de una OT
 */
...
/**
 * Obtiene lista de recursos disponibles (Mec├ínicos y Veh├¡culos)
 */
